-- Manual migration script to apply user discovery system
...
-- Step 2: Create direct connection requests table
CREATE TABLE IF NOT EXISTS public.direct_connection_requests (
...
CREATE INDEX IF NOT EXISTS idx_direct_requests_receiver ON public.direct_connection_requests(receiver_id) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_direct_requests_sender ON public.direct_connection_requests(sender_id);
CREATE INDEX IF NOT EXISTS idx_users_discovery ON public.users(visibility, last_active) WHERE visibility IN ('public', 'connections');
...
DROP POLICY IF EXISTS "Users can view requests they sent or received" ON public.direct_connection_requests;
CREATE POLICY "Users can view requests they sent or received" ON public.direct_connection_requests
FOR SELECT USING (
  auth.uid() = sender_id OR auth.uid() = receiver_id
);

DROP POLICY IF EXISTS "Users can create direct connection requests" ON public.direct_connection_requests;
...
DROP POLICY IF EXISTS "Users can update requests they sent or received" ON public.direct_connection_requests;
CREATE POLICY "Users can update requests they sent or received" ON public.direct_connection_requests
FOR UPDATE USING (
  auth.uid() = sender_id OR auth.uid() = receiver_id
);

-- Step 6: Create the send_direct_connection_request function
...
-- Step 7: Create the respond_to_direct_connection_request function
...
BEGIN
  ...
  IF p_response = 'accepted' THEN
    ...
    END IF;
  ELSE
    -- Create notification for sender about rejection (only if notifications table exists)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifications') THEN
      INSERT INTO notifications (
        user_id, type, title, message, data
      ) VALUES (
...
-- Step 8: Create the discover_users function
...
BEGIN
  ...
  SELECT
    ...
    u.skills,
    u.interests,
    -- Count mutual connections (simplified version)
...
-- Step 10: Add updated_at trigger (if not exists)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'trg_direct_connection_requests_updated_at'
    ) THEN
...
-- Step 11: Update existing users' last_active field
UPDATE public.users SET last_active = updated_at WHERE last_active IS NULL;

-- Step 12: Add some sample professional data for testing (optional)
-- Uncomment these lines if you want to add sample data to existing users
/*
UPDATE public.users SET
  company = CASE
    WHEN id = (SELECT id FROM auth.users LIMIT 1) THEN 'Tech Corp'
    ELSE 'Sample Company'
  END,
  role = CASE
    WHEN id = (SELECT id FROM auth.users LIMIT 1) THEN 'Software Engineer'
    ELSE 'Professional'
  END,
  location = 'San Francisco, CA',
  skills = ARRAY['JavaScript', 'React', 'Node.js'],
  interests = ARRAY['Technology', 'Networking', 'Innovation']
