...
const LeaderboardCard = ({ 
  ...
  return (
    <div 
      className={cn(
        "group relative bg-black rounded-[16px] md:rounded-[20px] border border-[#1a1a1a] hover:border-[#CBAA5A] overflow-hidden flex shadow-2xl transition-all duration-300 cursor-pointer",
        // Compact size for horizontal grid
        "w-full h-[220px] md:h-[240px]"
      )}
    >
      {/* Left Side - Content */}
      <div className="relative z-10 flex flex-col h-full p-3 w-[55%]">
        {/* Score Badge */}
        <div className="bg-gradient-to-br from-[#2a2a2a] via-[#1a1a1a] to-[#0f0f0f] rounded-lg p-2 border border-[#333] group-hover:border-[#CBAA5A]/50 w-fit mb-2 transition-colors duration-300">
          <div className="flex items-center gap-1 mb-0.5">
            <TrendingUp className="w-2.5 h-2.5 text-[#888] group-hover:text-[#CBAA5A] transition-colors duration-300" strokeWidth={2.5} />
            <span className="text-[7px] font-gilroy font-bold tracking-[0.15em] text-[#666] group-hover:text-[#CBAA5A]/70 uppercase transition-colors duration-300">
              SOCAP
            </span>
          </div>
          <div className={cn(
            "font-riccione text-[24px] md:text-[28px] leading-none tracking-tight group-hover:text-[#CBAA5A] transition-colors duration-300",
            tier.color
          )}>
            {user.social_capital_score}
          </div>
          <div className="text-[7px] font-gilroy font-bold tracking-[0.2em] text-[#555] group-hover:text-[#CBAA5A]/70 uppercase mt-0.5 transition-colors duration-300">
            {tier.name}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 mb-2">
          ...
          {user.position && (
            <span className="text-[8px] text-[#666] uppercase tracking-wider truncate">
              {user.position}
            </span>
          )}
        </div>
          ...
          <div className="mt-auto">
            <div className="flex gap-1.5">
              {user.featured_organizations.slice(0, 3).map((org, orgIndex) => (
                <div
                  key={orgIndex}
                  className="w-7 h-7 rounded-md bg-white/10 p-1 border border-[#333] group-hover:border-[#CBAA5A]/50 transition-colors duration-300 flex items-center justify-center"
                  title={org.name}
                >
                  <img
                    ...
                    onError={(e) => {
                      ...
                      target.parentElement!.innerHTML = `<span class="text-[#666] font-gilroy font-bold text-xs">${org.name[0]}</span>`;
                    }}
                  />
                </div>
              ))}
      ...
      <div className="relative w-[45%] h-full">
        ...
        <div className="absolute top-2 right-2 z-30">
          <div className="bg-[#1a1a1a]/90 backdrop-blur-sm rounded-full px-2 py-0.5 border border-[#333] group-hover:border-[#CBAA5A]/50 transition-colors duration-300">
            <span className="text-[9px] text-[#888] group-hover:text-[#CBAA5A] uppercase tracking-wider font-bold transition-colors duration-300">
              #{rank}
            </span>
          </div>
        </div>
