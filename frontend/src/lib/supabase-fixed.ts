// Fix for mobile black screen issues
// Add better error handling and mobile-specific fixes

// 1. Fix missing environment variables issue
// Update frontend/src/lib/supabase.ts to handle missing env vars gracefully
...
export const ensureAuthenticatedRequest = async () => {
  try {
    ...
    console.error('Error ensuring authenticated request:', error);
    throw error;
  }
};
...
export const authenticatedRpc = async <T = unknown>(
  ...
  try {
    ...
    // Fallback only for header/session-ish failures
    ...
    // Propagate original error for all other cases
    ...
    console.error('Error in authenticatedRpc:', error);
    throw error;
  }
};
