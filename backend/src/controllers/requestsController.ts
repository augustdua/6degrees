import { Response } from 'express';
import { AuthenticatedRequest } from '../types';
import { supabase } from '../config/supabase';

/**
 * DELETE /api/requests/:requestId/soft-delete
 * Soft delete a connection request
 */
export const softDeleteRequest = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    }

    const { requestId } = req.params;

    console.log(`🗑️ Soft deleting request ${requestId} for user ${userId}`);

    const { data, error } = await supabase.rpc('soft_delete_connection_request', {
      p_request_id: requestId
    });

    if (error) {
      console.error('❌ Error calling soft_delete_connection_request:', error);
      throw error;
    }

    console.log(`✅ Soft deleted request ${requestId}`);

    return res.status(200).json({ success: true, data });
  } catch (error: any) {
    console.error('Error in softDeleteRequest:', error);
    return res.status(500).json({ error: error.message || 'Internal server error' });
  }
};

