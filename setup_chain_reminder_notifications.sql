-- Setup chain reminder notifications system
-- Adapted to match your existing schema

-- 1) First, update the notifications type constraint to include 'chain_reminder'
ALTER TABLE public.notifications
DROP CONSTRAINT IF EXISTS notifications_type_check;

ALTER TABLE public.notifications
ADD CONSTRAINT notifications_type_check
CHECK (type IN ('chain_joined', 'target_claim', 'chain_approved', 'chain_rejected', 'reward_received', 'chain_reminder'));

-- 2) Add index for better performance on chain reminder queries
...
-- 3) Function to find chain tails that need reminders
-- Since you don't have link_shares table, we'll detect sharing by looking for children nodes
...
-- 4) Function to create chain reminder notifications
...
-- 6) Test the functions (optional - you can run this to test)
-- SELECT * FROM public.find_unshared_chain_tails(0, 0);  -- Find all current dead-ends
-- SELECT public.enqueue_chain_tail_reminders(0, 0);      -- Create test notifications