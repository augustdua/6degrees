// Fix for Supabase Authorization header issues
// This ensures the Authorization header is properly included in RPC calls
...
// Test function to verify auth headers are working
export const testAuthHeaders = async () => {
  try {
    const session = await ensureAuthenticatedRequest();
    console.log('✅ Auth headers test passed:', {
      userId: session.user.id,
      hasAccessToken: !!session.access_token
    });
    return true;
