...
-- This file contains the necessary database schema and functions for group chat functionality
...
CREATE INDEX IF NOT EXISTS idx_group_messages_chain_id ON group_messages(chain_id);
...
DROP POLICY IF EXISTS "Users can view group messages from their chains" ON group_messages;
DROP POLICY IF EXISTS "Users can send group messages to their chains" ON group_messages;

-- RLS Policy: Users can only see messages from chains they participate in
CREATE POLICY "Users can view group messages from their chains" ON group_messages
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM chains c
            WHERE c.id = group_messages.chain_id
            AND EXISTS (
                SELECT 1
...
-- RLS Policy: Users can only send messages to chains they participate in
CREATE POLICY "Users can send group messages to their chains" ON group_messages
    FOR INSERT
    WITH CHECK (
        sender_id = auth.uid()
        AND EXISTS (
...
-- 2. Function to get group chat messages
...
-- 3. Function to send group message
...
BEGIN
    ...
    -- Insert the message
    INSERT INTO group_messages (chain_id, sender_id, content)
    VALUES (p_chain_id, auth.uid(), trim(p_content))
...
-- 4. Function to get chain participants (optional - data is already in JSONB)
CREATE OR REPLACE FUNCTION get_chain_participants(p_chain_id UUID)
RETURNS TABLE (
    user_id UUID,
    email TEXT,
    first_name TEXT,
...
GRANT EXECUTE ON FUNCTION get_group_chat_messages TO authenticated;
GRANT EXECUTE ON FUNCTION send_group_message TO authenticated;
GRANT EXECUTE ON FUNCTION get_chain_participants TO authenticated;
...
-- 7. Create message_reactions table for group chat reactions
...
-- 8. Function to add message reaction
...
-- 9. Function to remove message reaction
...
-- 10. Enhanced function to get group chat messages with reactions
...
-- 11. Grant permissions for new tables and functions
